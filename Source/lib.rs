...
use crate::{
	proxy::service::ServiceProxy, session::Session, ss::SS_COLLECTION_LABEL, util::exec_prompt,
};
...
mod test {
	...
	async fn should_search_items() {
		...
		let search_item =
			ss.search_items(HashMap::from([("test_attribute_in_ss", "test_value")])).await.unwrap();
