...
//!
//! This library implements a rust interface to the Secret Service API which is
...
//! ### Entry point
//! The entry point for this library is the `SecretService` struct. A new
//! instance of `SecretService` will initialize the dbus connection and
//! negotiate an encryption session.
//!
//! ```
...
//! ```
//!
//! Once the SecretService struct is initialized, it can be used to navigate to
//! a collection. Items can also be directly searched for without getting a
//! collection first.
//!
//! ### Collections and Items
//! The Secret Service API organizes secrets into collections, and holds each
//! secret in an item.
//!
//! Items consist of a label, attributes, and the secret. The most common way to
//! find an item is a search by attributes.
//!
//! While it's possible to create new collections, most users will simply create
//! items within the default collection.
//!
//! ### Actions overview
//! The most common supported actions are `create`, `get`, `search`, and
//! `delete` for `Collections` and `Items`. For more specifics and exact method
//! names, please see each struct's documentation.
//!
//! In addition, `set` and `get` actions are available for secrets contained in
//! an `Item`.
//!
//! ### Crypto
...
//! ### Async
//!
//! This crate, following `zbus`, is async by default. If you want a synchronous
//! interface that blocks, see the [blocking] module instead.
// Util currently has interfaces (dbus method namespace) to make it easier to
// call methods. Util contains function to execute prompts (used in many
// collection and item methods, like delete)
...
impl<'a> SecretService<'a> {
	...
	pub async fn connect(
		...
		Ok(SecretService { conn, session, service_proxy })
	}
