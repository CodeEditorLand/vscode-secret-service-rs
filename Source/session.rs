...
// 1. Before session negotiation (openSession), set private key and public key
//    using DH method.
// 2. In session negotiation, send public key.
// 3. As result of session negotiation, get object path for session, which (I
//    think it means that it uses the same server public key to create an aes
//    key which is used to decode the encoded secret using the aes seed that's
...
// 6. Format Secret: aes iv is random seed, in secret struct it's the parameter
//    (Array(Byte))
// 7. Format Secret: encode the secret value for the value field in secret
//    struct. This encoding uses the aes_key from the associated Session.
...
impl Session {
	fn encrypted_session(keypair:&Keypair, session:OpenSessionResult) -> Result<Self, Error> {
		...
		Ok(Session { object_path:session.result, aes_key:Some(aes_key) })
	}
	...
	pub fn get_aes_key(&self) -> Option<&AesKey> { self.aes_key.as_ref() }
}
...
mod test {
	...
	// There is no async test because this tests that an encryption session can
