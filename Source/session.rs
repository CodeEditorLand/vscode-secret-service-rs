...
pub fn decrypt(encrypted_data: &[u8], key: &AesKey, iv: &[u8]) -> Result<Vec<u8>, Error> {
	...
	let iv = GenericArray::from_slice(iv);
	let cipher = Cbc::<Aes128, Pkcs7>::new_fix(key, iv);
	cipher.decrypt_vec(encrypted_data).map_err(|_| Error::Crypto("message decryption failed"))
}
...
pub fn decrypt(encrypted_data: &[u8], key: &AesKey, iv: &[u8]) -> Result<Vec<u8>, Error> {
	...
	ctx.cipher_final_vec(&mut output).map_err(|_| Error::Crypto("message decryption failed"))?;
	Ok(output)
}
